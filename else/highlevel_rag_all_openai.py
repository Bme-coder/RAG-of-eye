...
def run_query_engine(index):
    ...
    我们现在添加了基于 LLM (OpenAI) 的 Re-Ranking 步骤
    """
    print("--- 3. 启动查询引擎 (使用 OpenAI Re-Ranking 功能) ---")
    
    # 1. (新) 配置 Reranker
    # LlamaIndex 会自动使用您在 setup_global_settings() 中
    # 定义的 Settings.llm (即 gpt-4o-mini)
    reranker = LLMRerank(
        top_n=3, # 最终保留 3 个文档
    )
    ...
    query_engine = index.as_query_engine(
        # "广撒网": 先从向量库检索 10 个候选文档
        similarity_top_k=10, 
        
        # "智能过滤": 
        # 将我们基于 OpenAI 的 Reranker 作为后处理器添加
    ...
    while True:
        try:
            ...
            # LlamaIndex 现在会自动执行:
            # 1. 向量检索 (k=10)
            # 2. 调用 gpt-4o-mini 重排 (top_n=3)
            # 3. 调用 gpt-4o-mini 生成答案
            # 这会产生 API 费用！
